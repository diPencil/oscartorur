...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $hotelGatewayPaidIds = Deposit::where('user_id', $user->id)->where('status', Status::PAYMENT_SUCCESS)->whereNotNull('hotel_booking_id')->pluck('hotel_booking_id')->toArray();
        $manualHotelPaid = \App\Models\HotelBooking::where('user_id', $user->id)->where('payment_status', 'paid')
                                                   ->whereNotIn('id', $hotelGatewayPaidIds)
                                                   ->sum('total_price');
        $totalDeposit += $manualHotelPaid;

        $bookedTour = PlanLog::where('user_id', $user->id)->where('status', Status::PAYMENT_SUCCESS)->where('type', 'tour')->count();
        $bookedSeminar = PlanLog::where('user_id', $user->id)->where('status', Status::PAYMENT_SUCCESS)->where('type', 'seminar')->count();
        $bookedHotel = \App\Models\HotelBooking::where('user_id', $user->id)->count();

        $countries = json_decode(file_get_contents(resource_path('views/partials/country.json')));
        
        $agencies = \App\Models\Agency::where('status', 1)->get();

        return view('admin.users.detail', compact('pageTitle', 'user', 'totalDeposit', 'bookedTour', 'bookedSeminar', 'bookedHotel', 'countries', 'agencies'));
    }
