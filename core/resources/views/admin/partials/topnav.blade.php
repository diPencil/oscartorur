    ...
    <div class="navbar__right">
        <ul class="navbar__action-list">
            ...
            @php
                ...
                $activeLanguage = App\Models\Language::where('code', $langCode)->first();
                if(!$activeLanguage) $activeLanguage = $language->first();
            @endphp
            <li class="dropdown">
                <button type="button" class="primary--layer" data-bs-toggle="dropdown" data-display="static" aria-haspopup="true" aria-expanded="false" style="padding: 0 10px;">
                    <span class="text--base" style="font-weight: 500;">
                        @if($activeLanguage->image)
                            <img src="{{ getImage(getFilePath('language') . '/' . $activeLanguage->image, getFileSize('language')) }}" alt="flag" style="width: 20px; height: 14px; border-radius: 2px; object-fit: cover;">
                        @endif
                    </span>
                </button>
                <div class="dropdown-menu dropdown-menu--sm p-0 border-0 box--shadow1 dropdown-menu-right">
                    ...
                    <a href="{{ route('lang', $lang->code) }}" class="dropdown-menu__item d-flex align-items-center px-3 py-2">
                        @if($lang->image)
                            <img src="{{ getImage(getFilePath('language') . '/' . $lang->image, getFileSize('language')) }}" alt="flag" style="width: 20px; height: 14px; border-radius: 2px; margin-right: 10px; object-fit: cover;">
                        @endif
