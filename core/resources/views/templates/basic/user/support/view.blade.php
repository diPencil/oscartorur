...
@section('content')
                                ...
                                <div class="row border border-primary border-radius-3 my-3 py-3 mx-2">
                                    ...
                                    <div class="col-md-9">
                                        <small class="text-muted my-3">
                                            @lang('Posted on') {{ showDateTime($message->created_at, 'l, dS F Y @ h:i a') }}</small>
                                        <p>{{ $message->message }}</p>
                                        @if ($message->attachments->count() > 0)
                                            <div class="mt-2">
                                                @foreach ($message->attachments as $k => $image)
                                                    <a href="{{ route('ticket.download', encrypt($image->id)) }}" class="me-3 text--base"><i class="fa-regular fa-file"></i> @lang('Attachment') {{ ++$k }} </a>
                                                @endforeach
                                            </div>
                                        @endif
                                    </div>
    ...
    <x-confirmation-modal closeBtn="btn btn--dark btn-sm" submitBtn="btn btn--base btn-sm"/>
@endsection
@push('style')
    <style>
